...
Our curation procedure differed slightly for the reduced entire datasets, as we downloaded unaligned sequences from GISAID to generate the reduced dataset, but downloaded aligned sequences to generate the entire dataset.
...
By contrast, processing of the entire dataset (where we started from aligned sequences), we first applied a single filter using the script `scripts/sequence_data_curation/sequence_process_gisaidalignment.R`.
These scripts also trim the alignment after filtration to generate nucleotide alignments that only retain coding regions.

After the initials curation, we then defined data subsets within the curated alignments according to various partition schemes by running the script `scripts/sequence_data_curation/sequence_process_alignment_foranalysis.R`; these partition schemes include ,*e.g.*, subdividing the alignments by codon position and/or by gene regions, to allow different substitution models to be specified for subsets of the alignment to capture variation in the substitution process across the genomes.
The script `scripts/sequence_data_curation/discretetrait_writer.R` generates a data table that contains the sampling date (in unit of days) and sampling locality (according to the discrete geographic areas defined in `scripts/sequence_data_curation/main_config.R`)..
