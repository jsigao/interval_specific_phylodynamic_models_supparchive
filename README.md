...
It is divided into four directories: the [`data`](#data) directory contains the genomic, geographic and sampling data used in our phylodynamic analyses, and also includes data on human-travel volume and public-health measures; the [`analyses`](#analyses) directory contains the XML scripts specifying the phylodynamic analyses we performed using the `BEAST` software package; the [`scripts`](#scripts) directory contains the `R` scripts that we used to curate data and to postprocess the output from our analyses, and; the [`program`](#program) directory contain a modified version of the `BEAST` software package that implements various extensions required to perform some of the phylodynamic analyses in our study. The details of each subdirectory are described below.
