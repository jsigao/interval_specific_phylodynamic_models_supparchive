...
#' 100 days old and we want to count the number of events per day, then this vector needs to be 0:100
#' @return a list of vectors, where each vector corresponds to a time interval (the first vector represents the last time interval);
#' each element of each vector is the number active lineages in the given time interval in the corresponding tree
get_nlineages_perinterval <- function(trees, epoch_bounds) {
  
  epoch_num <- length(epoch_bounds)
  nlineages_epoch <- vector("list", epoch_num)
  tree_num <- length(trees)
  
  for (m in 1:epoch_num) { # loop over time intervals
    nlineages <- integer(tree_num)
    
    if (m < epoch_num) {
      epoch_bound <- c(epoch_bounds[m], epoch_bounds[m + 1])
      age_bound_left <- epoch_bounds[m + 1]
      age_bound_right <- epoch_bounds[m]
    } else if (m == epoch_num) { # the most ancient interval
      epoch_bound <- c(epoch_bounds[m], Inf)
      age_bound_left <- Inf
      age_bound_right <- epoch_bounds[m]
    }
    
    for (k in 1:tree_num) { # loop over trees
      tree <- trees[[k]]
      node_ages <- max(node.depth.edgelength(tree)) - node.depth.edgelength(tree)
      edge_age <- cbind(node_ages[tree$edge[, 1]], node_ages[tree$edge[, 2]])
      nlineages[k] <- sum(edge_age[, 1] >= age_bound_right & edge_age[, 2] < age_bound_left)
    }
    
    nlineages_epoch[[m]] <- nlineages
  }
  
  return(nlineages_epoch)
}