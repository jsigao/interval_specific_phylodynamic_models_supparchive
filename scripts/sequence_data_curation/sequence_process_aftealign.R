# this script reads in a sequence alignment (assumed to be aligned locally)
# and then perform the second round of filteration to generate an alignment to use in the inference
library(ape)
library(seqinr)
...
alignment <- read.dna(paste0("./data/sequence_data/alignments/not_partitioned/ncov_", submission_dateby, "_", collection_dateby, "_aligned.fasta"),
                      format = "fasta", as.character = T)
sequence_accession <- as.character(sapply(rownames(alignment), function(x) unlist(strsplit(x, "\\|"))[2]))
...
for (i in 1:nrow(alignment_53primeUTRtrimmed)) {
  ...
  numsites_diff_toref[i] <- sum(sapply(1:ncol(alignment_53primeUTRtrimmed), function(j) alignment_53primeUTRtrimmed_current[j] != reference_inalignment_53primeUTRtrimmed[j]))
}
...
toomuch_missing_exclude <- rownames(alignment_53primeUTRtrimmed)[alignment_53primeUTRtrimmed_missing_num >= 15 | 
                                                                   (alignment_53primeUTRtrimmed_missing_num >= 9 & 
...
virus_name <- sapply(strsplit(rownames(alignment), "\\|"), "[[", 1)
if (any(duplicated(virus_name))) {
  ...
  for (i in 1:length(virus_name_dup)) {
    ...
    accession_id_num <- as.integer(gsub("EPI_ISL_", "", sapply(strsplit(sequence_name_dup, "\\|"), "[[", 2)))
    duplicate_exclude <- c(duplicate_exclude, sequence_name_dup[accession_id_num == min(accession_id_num)])
  }
...
write.dna(alignment, file = paste0("./data/sequence_data/alignments/not_partitioned/ncov_", submission_dateby, "_", collection_dateby, "_aligned_trimmed.fasta"), 
...
seqinr::write.fasta(sequences = lapply(1:nrow(alignment_aa), function(i) alignment_aa[i, ]), names = rownames(alignment_aa), 
                    file.out = paste0("./data/sequence_data/alignments/not_partitioned/ncov_", submission_dateby, "_", collection_dateby, "_aligned_aa.fasta"))
write.dna(alignment_codon, file = paste0("./data/sequence_data/alignments/not_partitioned/ncov_", submission_dateby, "_", collection_dateby, "_aligned_codon.fasta"), 
          format = "fasta", nbcol = -1, colsep = "", colw = 3.2e4)
